...
## Running with Watchtower (Auto-Updates)

Watchtower automatically updates your validator when new Docker images are pushed.
...
services:
  validator:
    image: ghcr.io/YOUR_ORG/templar-validator:latest
    container_name: templar-validator
  ...
  watchtower:
    ...
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
    command: --interval 300 templar-validator
...
```bash
# Start validator with watchtower
docker compose up -d
...
docker inspect templar-validator --format='{{.Created}}'

# View watchtower update history
...
| `--include-stopped` | Update stopped containers too |
| `--notifications-level info` | Notification verbosity |
