...
Usage:
    1. Run setup: uv run python local_test/setup_benchmark.py
    2. Test locally: cd local_test && uv run python train.py
    3. Submit when ready!
...
if __name__ == "__main__":
    ...
    print(f"Configuration from hparams.json:")
    ...
    if not model_path.exists():
        ...
        print("   Run: uv run python local_test/setup_benchmark.py")
        exit(1)
    ...
    for eval_num in range(num_evals):
        ...
        print(f"   Eval {eval_num + 1}/{num_evals}: {tps:,.0f} TPS ({wall_time:.2f}s, loss={result.final_loss:.4f})")
